...
/**
 * @author pedro
 *	Clase que realiza el Tagueo, lo cual consiste en buscar los incidentes en RWD, Con los resultados obtenidos,
 * buscar las llamadas en un rango de tiempo, a los resultados encontrados se les debe poner el número de incidente 
 * y se debe actualizar la sesion
 */
	...
	public void procesar() throws SQLException {
		...
		for(LlamadaDTO llamada : llamadas) {
			...
			try {
				...
				sesiones = consultaVerintService.consultarVerint(llamada, TOKEN);
				LOG.info("Despues Consulta Verint Exception: " + (sesiones !=null ? sesiones.size() + "": "0") + " sesiones ");
			}
